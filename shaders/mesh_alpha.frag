...
layout(binding = 0, set = 2) uniform samplerCube u_irradiance_map;

layout(binding = 0, set = 3) uniform sampler2D u_integral_lookup;
layout(binding = 0, set = 4) uniform samplerCube u_prefilter_map;

layout(binding = 0, set = 5) uniform sampler2D u_ao;

layout(binding = 0, set = 1) uniform lighting_t {
...
layout(binding = 0, set = 0) uniform camera_transforms_t {
    mat4 projection;
    mat4 view;
    mat4 inverse_view;
...
void main() {
    vec4 raw_albedo = u_push_constant.color;
    vec4 raw_vs_normal = vec4(in_fs.vs_normal, 0.0f);
    vec3 vs_position = in_fs.vs_position;
